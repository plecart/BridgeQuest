...
class AuthServiceTestCase(TestCase):
    ...
    def test_create_or_get_user_from_sso_data_no_email_raises_exception(self):
        ...
        sso_data = {"given_name": "Test", "family_name": "User"}
...
class GoogleAuthServiceTestCase(TestCase):
    ...
    def _assert_google_user_data(
        self, result, email, given_name, family_name, picture, sub
    ):
...
class AppleAuthServiceTestCase(TestCase):
    ...
    def _assert_apple_user_data(
        self, result, email, sub, given_name="", family_name=""
    ):
    ...
    @patch("accounts.services.apple_auth_service.jwt.get_unverified_header")
    def test_validate_apple_token_invalid_token_error_raises_bridgequest(
    ...
    @patch("accounts.services.apple_auth_service._decode_and_verify_token")
    def test_validate_apple_token_generic_exception_raises_auth_sso_failed(
        self, mock_decode
    ):
    ...
    def test_find_matching_public_key_success(self, mock_from_jwk):
        ...
        result = apple_auth_service._find_matching_public_key(
            apple_keys, unverified_header
        )

        self.assertEqual(result, mock_key)
        # PyJWT RSAAlgorithm.from_jwk attend une chaîne JSON JWK (CODING_STANDARDS)
        expected_jwk = json.dumps({"kid": "APPLE_KEY_ID"})
        mock_from_jwk.assert_called_once_with(expected_jwk)
    ...
    def test_decode_token_with_key_success(self, mock_decode, mock_settings):
        ...
        result = apple_auth_service._decode_token_with_key(
            self.valid_token, mock_public_key
        )
